...
To install `fsr` locally from the project directory, navigate to the `congregation_reporter` directory (where `setup.py` is located) and run:

```bash
pip install .
```

If the package were published to PyPI, it could be installed via:
```bash
pip install fsr
```
