...
def prepare_reports_lookup(reports_data):
    ...
    if reports_data:
        for report in reports_data:
            ...
            user_data = report.get('user', {}) # Get the user dictionary, default to empty if 'user' key is missing
            user_id = user_data.get('id')      # Get 'id' from the user_data

            if user_id is not None and year is not None and month is not None:
                try:
                    # Ensure year and month are integers for the key
